...
const KurtasAndSuits = () => {
  const [isKurtasAndSuits, setIsKurtasAndSuits] = useState([]);
  const [isLoading, setIsLoading] = useState(false);

  const getKutasAndSuitsProducts = async () => {
    try {
      setIsLoading(true)
      const response = await getReq(`/products?category=kurtas-and-suits&limit=12`);
      setIsKurtasAndSuits(response?.products || []);
    } catch (error) {
      console.log(error);
    } finally {
      setIsLoading(false)

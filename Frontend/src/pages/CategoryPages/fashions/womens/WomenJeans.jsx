...
const WomenJeans = () => {
  const [isWomenJeans, setIsWomenJeans] = useState([]);
  const [isLoading, setIsLoading] = useState(false);

  const getJeansProducts = async () => {
    try {
      setIsLoading(true)
      const response = await getReq(`/products?category=women-jeans&limit=12`);
      setIsWomenJeans(response?.products || []);
    } catch (error) {
      console.log(error);
    } finally {
      setIsLoading(false)
